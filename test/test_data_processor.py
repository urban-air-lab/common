...
def sample_targets():
    ...
    return pd.DataFrame({"NO2": np.random.rand(10)}, index=dates)
...
def test_no_gases_list_raises_value_error():
    df = pd.DataFrame({"RAW_ADC_NO_W": [1], "RAW_ADC_NO_A": [0]})
...
def test_align_dataframes_by_time():
    ...
    df2 = pd.DataFrame(
        data={"v2": list(range(7, 14))},
        index=pd.date_range("2021-01-04", periods=7, freq="D"),
    )
...
def test_align_dataframes_by_time_returns_empty_dataframes():
    df1 = pd.DataFrame(
        data={"v1": [1, 2, 3]}, index=pd.date_range("2021-01-01", periods=3, freq="D")
    )
    df2 = pd.DataFrame(
        data={"v2": [4, 5, 6]}, index=pd.date_range("2021-01-10", periods=3, freq="D")
    )
...
def test_align_dataframes_by_time_string_and_datetime_index_conversion():
    ...
    df1 = pd.DataFrame(
        data={"v1": [10, 20, 30]}, index=["2021-06-01", "2021-06-02", "2021-06-03"]
    )
    df2 = pd.DataFrame(
        data={"v2": [100, 200, 300]},
        index=pd.to_datetime(["2021-06-02", "2021-06-03", "2021-06-04"]),
    )
